...
from models import engine, Base, UserModel, Session
from middlewares import session_middleware

app = web.Application()


async def orm_context(app: web.Application):   # создаем экземпляр класса приложения
    print('START')
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    yield            # все то что до yield произойдет на старте приложения
...
app.add_routes([
    web.get('/user/{art_id:\d+}', UserView),
    web.patch('/user/{art_id:\d+}', UserView),
    web.delete('/user/{art_id:\d+}', UserView),
